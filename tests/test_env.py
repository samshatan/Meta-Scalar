...
class TestTask1:
    ...
    def test_grader_correct_episode(self, env):
        env.reset("alert_classification", 0)
        # Investigate then classify correctly in 2 steps
        ...
        assert result["score"] > 0.5  # correct + efficient
...
class TestTask2:
    ...
    def test_full_episode_partial_credit(self, env):
        ...
        assert done
        result = env.grade()
        # Should have partial credit for investigation + classification + resolution
...
class TestTask3:
    ...
    def test_full_optimal_episode(self, env):
        ...
        # Investigate root cause services
        ...
        result = env.grade()
        assert result["score"] >= 0.75  # good episode should score well
        assert result["breakdown"]["classification"] == 1.0
    ...
    def test_wrong_remediation_penalised(self, env):
        ...
        _, reward, _, _ = env.step(Action(
            action_type=ActionType.REMEDIATE,
            service_name="redis-cluster",
            remediation_action=RemediationAction.SCALE_UP,  # wrong
        ))
        assert reward.score < 0  # penalty for wrong remediation
    ...
    def test_step_budget_enforced(self, env):
        ...
        while not done and steps < 25:
            ...
            steps += 1
        assert done
        assert steps <= 21  # max_steps=20 + 1
...
class TestSpecCompliance:
    ...
    def test_cumulative_reward_non_decreasing_good_actions(self, env):
        ...
        for action in good_actions:
            ...
            assert reward.cumulative >= prev - 0.001  # allow fp tolerance
