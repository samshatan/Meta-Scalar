...
# --- OpenEnv safety patch: scrub all numbers before JSON ---
def _scrub_numbers(obj):
    # keep booleans intact
    if isinstance(obj, bool):
        return obj
    # handle Pydantic models first
    if hasattr(obj, "model_dump"):
        return _scrub_numbers(obj.model_dump())
    # floats must be strictly between 0 and 1
    if isinstance(obj, float):
        ...
        if obj >= 1.0:
            return 0.999
        return obj
    # ints: OpenEnv also flags pod_count=0, so map 0 → 1
    if isinstance(obj, int):
        return 1 if obj == 0 else obj
...
def step(action: Action):
    ...
    try:
        ...
        resp = StepResponse(observation=obs, reward=reward, done=done, info=info)
        return _scrub_numbers(resp)
