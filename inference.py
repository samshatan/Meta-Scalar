...
try:
    ...
    OpenAI = None  # Will error at runtime if used
...
class BaselineAgent:
    def __init__(self, model: str = MODEL_NAME):
        if not API_KEY:
            raise RuntimeError("API_KEY or HF_TOKEN environment variable not set.")
        if OpenAI is None:
...
def obs_to_text(obs_dict: Dict[str, Any]) -> str:
    ...
    if obs_dict.get("visible_logs"):
        lines.append("\n=== RECENT LOGS ===")
        for l in obs_dict["visible_logs"][-20:]:  # last 20 log lines
...
def _heuristic_action(obs_dict: Dict, task_id: str, step: int) -> Dict:
    ...
    # Step 4: remediate on first investigated service
