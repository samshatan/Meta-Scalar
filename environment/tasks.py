...
# ─────────────────────────────────────────────────────────────────────────────
# The agent receives a single alert + initial service health snapshot.
# It must call classify(category) with the correct IncidentCategory.
# Reward is immediate and binary (0 or 1) for this easy task, but there
# is a step-efficiency bonus for classifying quickly.
...
# services before resolving, classify correctly, and identify the root service.
# Partial credit for partial investigation; full credit for correct resolution.
...
# ─────────────────────────────────────────────────────────────────────────────
# The agent must: classify → investigate (≥3 services) → apply the correct
# remediation → verify → resolve with a meaningful summary.
# Grader rewards each phase independently with partial credit.
...
TASK_3_SCENARIOS = [
    {
        ...
        "ground_truth": {
            # Both redis-cluster AND ml-feature-store are root causes (disk exhaustion)
...
def grade_task1(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Efficiency: how many steps were used
...
def grade_task2(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Root service identified (proved by investigating it)
